...
func createAciExternalManagementNetworkInstanceProfile(attributes map[string]interface{}) map[string]Item {
	...
	// Process MgmtRsOoBCons child objects
...
func createAciVrfFallbackRouteGroup(attributes map[string]interface{}) map[string]Item {
	...
	//children := item["fvFBRGroup"].Children
	//children = append(children, map[string]Item{"tagAnnotation": {Attributes: tagAnnotationChildren}})
...
func createAciNetflowMonitorPolicy(attributes map[string]interface{}) map[string]Item {
	...
	// Process NetflowRsMonitorToExporter child objects
	...
	// Process NetflowRsMonitorToRecord child objects
...
func main() {
	...
	log.Fatalf("Failed to create matching item list and final tree after 50 attempts")
}
