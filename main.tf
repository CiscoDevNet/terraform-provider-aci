...
resource "aci_tenant" "tenant_for_epg_test" {
  name        = "tenant_for_epg_test"
  description = "This tenant is created by terraform ACI provider"
}

resource "aci_application_profile" "app_profile_for_epg" {
  tenant_dn   = "${aci_tenant.tenant_for_epg_test.id}"
  name        = "ap_for_epg"
...
output "demo1" {
  value = "${data.aci_contract_consumer_provider.example}"
}
...
# resource "aci_domain" "foodomain" {
#   attachable_access_entity_profile_dn = "${aci_attachable_access_entity_profile.tst1.id}"
#   t_dn = "${aci_tenant.test_tenant1.id}"
# }
...
# resource "aci_node_policy_end_point" "foonode_policy_end_point" {
#   vpc_explicit_protection_group_dn = "${aci_vpc_explicit_protection_group.foovpc_explicit_protection_group.id}"
#   node_policy_end_point_id = "1"
# }
