...
| `migration_version` | 23 | `class.state_upgrades[].prior_schema_version` (+ `class.migration_source: from_sdkv2`) | One `state_upgrades` entry per declared version. See §2.1 for the two-source merge with [schema-git-commit-e21fb3e5.json](../definitions/schema-git-commit-e21fb3e5.json). |
| `migration_blocks` | 18 | `class.state_upgrades[].attributes` / `.children` | Each `oldKey: newKey` pair becomes an `AttributeUpgradeDefinition` with `legacy_attribute: oldKey` keyed by meta camelCase property name. Dotted values (e.g. `relation_to_static_leafs.deployment_immediacy`) become inner `attributes` / `children` entries. The `legacy_type` and `legacy_restriction` fields are read from [schema-git-commit-e21fb3e5.json](../definitions/schema-git-commit-e21fb3e5.json) and omitted when they match the current property. See §2.1. |
...
The three SDKv2-migration keys (`migration_version`, `migration_blocks`, `type_changes`) translate into a single `state_upgrades:` tree on the new class definition, but they need a **second input file** to be lossless. The legacy YAML only carries name pairs (old snake_case ↔ new attribute name); the prior framework attribute type and the prior `{required, optional, computed}` triplet live in [gen/definitions/schema-git-commit-e21fb3e5.json](../definitions/schema-git-commit-e21fb3e5.json) — a one-shot `terraform providers schema -json` dump of the SDKv2 provider, frozen at git commit `e21fb3e5`. The legacy generator opens it at every run via the [schema-git-commit-* glob](https://github.com/CiscoDevNet/terraform-provider-aci/blob/v2.19.0/gen/generator.go#L1157) and stores the parsed map at `Definitions.Migration`, where [SetMigrationClassTypes](https://github.com/CiscoDevNet/terraform-provider-aci/blob/v2.19.0/gen/generator.go#L2517) and [SetLegacyAttributes](https://github.com/CiscoDevNet/terraform-provider-aci/blob/v2.19.0/gen/generator.go#L2576) read it.

The migration script inlines the JSON data into each class's `state_upgrades:` tree at migration time, after which **the JSON file is no longer referenced** — nothing in the new pipeline reads it (`grep -RnE 'schema-git-commit|Definitions.Migration|provider_schemas' gen/utils gen/templates gen/scripts` returns zero hits). The file can be deleted from `gen/definitions/` after migration completes.
...
| Cross-field validator | A non-zero `migration_source` requires at least one `state_upgrades` entry (otherwise the docs would say "migrated" but `UpgradeResourceState` would have nothing to do). | [class.go:1922](../utils/data/class.go) `validateStateUpgrades` |
...
|---|---|---|
| `Class.Parents` (the parent class set) | [setParents](../utils/data/class.go) (class.go:436) | meta `containedBy` ∪ `class.include_parents`, minus `class.exclude_parents` ∪ `global.exclude_parents`. |
| Top-level `TestDependency{Role: Parent}` (first **2** parent classes) | [resolveParentDependencies](../utils/data/class.go) (class.go:1004) | First parent → `aci_<resource>.test.id` + `aci_<resource>.test_2.id` (two instances for ForceNew testing). Second parent → `aci_<resource>.test.id`. Additional parents are skipped — provide them explicitly if needed. |
| Top-level `TestDependency{Role: Target}` (single-target only) | [resolveTargetDependencies](../utils/data/class.go) (class.go:1035) | `Relation.ToClasses[0]` → `aci_<resource>.test.id` + `aci_<resource>.test_2.id`. **Multi-target relations (`len(Relation.ToClasses) > 1`) raise a diagnostic** unless an explicit `role: target` dependency is declared. |
| Recursive `TestDependency.Dependencies` (the parent's own parents) | [buildDependency](../utils/data/class.go) (class.go:1083) | For each auto-built dep, recurses through `depClass.Parents` and emits `aci_<resource>.test.id` for each. |
| `parent_dn` attribute test values | [setParentDn](../utils/data/class.go) (class.go:1172) | Wires `Create` / `Update` / `Default` from the first Parent dep; `ForceNew` from the second. Skipped if the property has an explicit `test_config`. |
| `tDn` / `tn<Cap>Name` attribute test values | [setTargetDn](../utils/data/class.go) (class.go:1239) / [setTargetNameProperty](../utils/data/class.go) (class.go:1306) | Explicit relations expose `tDn` (full DN); Named relations expose `tn<TargetCap>Name`. Wired from the auto-resolved Target dep. |
...
3. If `parent_dn` is a static literal (e.g. `uni/infra`), emit `test_config.dependencies[]` with `class_name`, `reference: <literal>`, `reference_type: static`, `role: parent`. **Future optimisation — keep the dep for now.** The static dep is only needed because [setParentDn](../utils/data/class.go) (class.go:1172) returns early when no Parent dep exists, leaving `parent_dn.TestValues` nil — the dep is what populates the Create/Update/Default `TestValueEntry` with `ConfigInclude: true` so the generated HCL carries an explicit `parent_dn = "uni/infra"`. When the property already carries `default_values: { "uni/infra": "" }`, the optional-property branch in [generateDefaultTestValues](../utils/data/property.go) (property.go:730-749) already produces a valid test (config-omit + assert-default), so the static dep is redundant for correctness — the class doesn't need to be created either, since `uni/infra` is a system DN. Migration keeps the dep for parity with the legacy YAML; a future change to `setParentDn` that falls back to `default_values` when no Parent dep is present would let `migrate_class_definitions.go` drop the static dep for any class whose `default_values.parent_dn` already covers the value. Tracked here so the optimisation surfaces when the test renderer is touched.
...
| `properties/resource_name_overwrite.yaml` (entire file) | 1 | The 7 relation → resource-name entries are now expressed as per-class `resource_name` on each relation class definition. |
...
[gen/definitions/properties/topSystem.yaml](../definitions/properties/topSystem.yaml) was authored alongside the manual plugin-framework migration of `aci_system` as a placeholder for the day the generator can produce this datasource. Today nothing reads it: the top-level `datasource_required:` list is not in the closed set of keys the legacy generator recognises (the matching template lookup `$.datasource_required` reads the unrelated `test_values.datasource_required` *bucket*, not the top-level list), and the new pipeline has no consumer either.
...
- [resolveParentDependencies](../utils/data/class.go) (class.go:1004) — relax the 2-parent cap when polymorphic; emit all N parents.
- [resolveTargetDependencies](../utils/data/class.go) (class.go:1035) — replace the multi-target diagnostic with a polymorphic branch that emits one target dep per `ToClass`, with distinct HCL labels (e.g. `aci_<resource>.test_master.id`) so parent and target of the same class don't collide in HCL.
- [setParentDn](../utils/data/class.go) (class.go:1172) / [setTargetDn](../utils/data/class.go) (class.go:1239) — bail when polymorphic; the renderer iterates scenarios instead of receiving single-step wiring.
- [buildTestChildren](../utils/data/class.go) (class.go:1488) — when the child class is polymorphic-same-type and the calling parent is in its `ToClasses`, override `tDn` in the built instance with the target dep matching the parent's class. This replaces the per-EPG `exclude_targets` lists for child rendering.
...
The current [migrate_class_definitions.go](migrate_class_definitions.go) handles 5 class-level direct keys (`allow_delete`, `exclude_children`, `include_children`, `sub_category`, `ui_locations`) and writes per-class output files. Every other section of this overview is decided; the work that remains is implementation, plus three small upstream changes outside the script itself.

Script inputs are the existing YAML files under [gen/definitions/](../definitions/) plus the one-shot SDKv2 schema dump [gen/definitions/schema-git-commit-e21fb3e5.json](../definitions/schema-git-commit-e21fb3e5.json). The JSON is consumed only when translating §2's three SDKv2-migration keys (see §2.1) and can be deleted from the repository after migration completes — nothing in the new pipeline reads it.

### 9.1 Code changes required *before* the script runs end-to-end

The script writes YAML; consuming it requires three groups of upstream changes.
...
| `GlobalMetaDefinition.PropertyDocumentationOverrides` | Consumed directly by the renderer; no per-class resolved counterpart needed. | n/a (lookup is per-property at render time). | Insert a global-lookup branch into [setDescription](../utils/data/property_documentation.go) between the per-class override and the meta-comment fallback; interpolate `%s` with the class's humanised resource name (same `GetResourceNameAsDescription` substitution the legacy generator uses). |
...
2. **§2 semantic mapping, class-level** — implement the shape-changing transforms: `resource_notes` (+ `resource_warnings` / `datasource_notes` / `datasource_warnings` sibling slots, no v2.19.0 data), `children` → `include_children`, `contained_by` → `include_parents` (subtract meta `containedBy` first), `class_version` → `supported_versions`, `relationship_classes` (+ drop `multi_relationship_class`) → `relation_info.to_classes`, and `migration_version` + `migration_blocks` + `type_changes` → `state_upgrades` (two-source merge with [schema-git-commit-e21fb3e5.json](../definitions/schema-git-commit-e21fb3e5.json), see §2.1; sets `migration_source: from_sdkv2` and omits `legacy_type` / `legacy_restriction` when they match the current property).
...
9. **§8 POSTPONE** — preserve the 6 entries unchanged with a `# TODO(postpone-§8.N): …` marker so the loader’s strict unmarshal doesn’t trip and so the marker is greppable when the dependent work lands. Add the matching keys to the loader as `yaml:"-"` placeholders or move them into a side-file the loader explicitly ignores until the disposition is taken.
...
Once §1 + §2 are covered, the new loader’s `UnmarshalStrict` will surface any remaining unknown keys; given the walk-through above, those should be limited to the §8 POSTPONE markers and will confirm coverage rather than reveal new decisions.
...
| Property-level YAML keys declared in files | 20 (+ 7 entries inside `properties/resource_name_overwrite.yaml`) | §1, §2, §3, §4, §5, §6, §8 |
