...
func createFvFBRoute(attributes map[string]interface{}) map[string]interface{} {
	...
	if v, ok := attributes["fbr_prefix"].(string); ok && v != "" {
	...
	newAciFvFBRoute := provider.GetFvFBRouteCreateJsonPayload(ctx, &diags, data, planTagAnnotation, planTagAnnotation, planTagTag, planTagTag)
