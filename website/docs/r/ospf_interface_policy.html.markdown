...
* `ctrl` - (Optional) Interface policy controls. Allowed values are "unspecified", "passive", "mtu-ignore", "advert-subnet" and "bfd". Default is "unspecified". (Multiple Comma-Delimited values are allowed. E.g., "bfd,passive"). 
