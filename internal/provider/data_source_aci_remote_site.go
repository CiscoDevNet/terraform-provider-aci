...
func (d *FvRemoteIdDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "Start metadata of datasource: aci_remote_site")
	resp.TypeName = req.ProviderTypeName + "_remote_site"
	tflog.Debug(ctx, "End metadata of datasource: aci_remote_site")
}

func (d *FvRemoteIdDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "Start schema of datasource: aci_remote_site")
	resp.Schema = schema.Schema{
		// This description is used by the documentation generator and the language server.
		MarkdownDescription: "The remote_site datasource for the 'fvRemoteId' class",
	...
	tflog.Debug(ctx, "End schema of datasource: aci_remote_site")
}
...
func (d *FvRemoteIdDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "Start configure of datasource: aci_remote_site")
	...
	d.client = client
	tflog.Debug(ctx, "End configure of datasource: aci_remote_site")
}

func (d *FvRemoteIdDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "Start read of datasource: aci_remote_site")
	...
	tflog.Debug(ctx, fmt.Sprintf("Read of datasource aci_remote_site with id '%s'", data.Id.ValueString()))

	getAndSetFvRemoteIdAttributes(ctx, &resp.Diagnostics, d.client, data)

	if data.Id.IsNull() {
		resp.Diagnostics.AddError(
			"Failed to read aci_remote_site data source",
			fmt.Sprintf("The aci_remote_site data source with id '%s' has not been found", cachedId),
		)
		return
	}
	...
	tflog.Debug(ctx, fmt.Sprintf("End read of datasource aci_remote_site with id '%s'", data.Id.ValueString()))
}
