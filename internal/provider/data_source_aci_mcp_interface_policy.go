...
var _ datasource.DataSource = &McpIfPolDataSource{}

func NewMcpIfPolDataSource() datasource.DataSource {
...
func (d *McpIfPolDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "Start metadata of datasource: aci_mcp_interface_policy")
	resp.TypeName = req.ProviderTypeName + "_mcp_interface_policy"
	tflog.Debug(ctx, "End metadata of datasource: aci_mcp_interface_policy")
}

func (d *McpIfPolDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "Start schema of datasource: aci_mcp_interface_policy")
	resp.Schema = schema.Schema{
		// This description is used by the documentation generator and the language server.
		MarkdownDescription: "The mcp_interface_policy datasource for the 'mcpIfPol' class",
	...
	tflog.Debug(ctx, "End schema of datasource: aci_mcp_interface_policy")
}

func (d *McpIfPolDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "Start configure of datasource: aci_mcp_interface_policy")
	...
	d.client = client
	tflog.Debug(ctx, "End configure of datasource: aci_mcp_interface_policy")
}

func (d *McpIfPolDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "Start read of datasource: aci_mcp_interface_policy")
	...
	// Create a copy of the Id for when not found during getAndSetMcpIfPolAttributes
	cachedId := data.Id.ValueString()

	tflog.Debug(ctx, fmt.Sprintf("Read of datasource aci_mcp_interface_policy with id '%s'", data.Id.ValueString()))

	getAndSetMcpIfPolAttributes(ctx, &resp.Diagnostics, d.client, data)

	if data.Id.IsNull() {
		resp.Diagnostics.AddError(
			"Failed to read aci_mcp_interface_policy data source",
			fmt.Sprintf("The aci_mcp_interface_policy data source with id '%s' has not been found", cachedId),
		)
		return
	}
	...
	tflog.Debug(ctx, fmt.Sprintf("End read of datasource aci_mcp_interface_policy with id '%s'", data.Id.ValueString()))
}
