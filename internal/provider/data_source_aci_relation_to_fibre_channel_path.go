...
var _ datasource.DataSource = &FvRsFcPathAttDataSource{}
...
func (d *FvRsFcPathAttDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "Start metadata of datasource: aci_relation_to_fibre_channel_path")
	resp.TypeName = req.ProviderTypeName + "_relation_to_fibre_channel_path"
	tflog.Debug(ctx, "End metadata of datasource: aci_relation_to_fibre_channel_path")
}

func (d *FvRsFcPathAttDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "Start schema of datasource: aci_relation_to_fibre_channel_path")
	resp.Schema = schema.Schema{
		// This description is used by the documentation generator and the language server.
		MarkdownDescription: "The relation_to_fibre_channel_path datasource for the 'fvRsFcPathAtt' class",
	...
	tflog.Debug(ctx, "End schema of datasource: aci_relation_to_fibre_channel_path")
}
...
func (d *FvRsFcPathAttDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "Start configure of datasource: aci_relation_to_fibre_channel_path")
	...
	d.client = client
	tflog.Debug(ctx, "End configure of datasource: aci_relation_to_fibre_channel_path")
}

func (d *FvRsFcPathAttDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "Start read of datasource: aci_relation_to_fibre_channel_path")
	...
	// Create a copy of the Id for when not found during getAndSetFvRsFcPathAttAttributes
	cachedId := data.Id.ValueString()

	tflog.Debug(ctx, fmt.Sprintf("Read of datasource aci_relation_to_fibre_channel_path with id '%s'", data.Id.ValueString()))

	getAndSetFvRsFcPathAttAttributes(ctx, &resp.Diagnostics, d.client, data)

	if data.Id.IsNull() {
		resp.Diagnostics.AddError(
			"Failed to read aci_relation_to_fibre_channel_path data source",
			fmt.Sprintf("The aci_relation_to_fibre_channel_path data source with id '%s' has not been found", cachedId),
		)
		return
	}
	...
	tflog.Debug(ctx, fmt.Sprintf("End read of datasource aci_relation_to_fibre_channel_path with id '%s'", data.Id.ValueString()))
}
