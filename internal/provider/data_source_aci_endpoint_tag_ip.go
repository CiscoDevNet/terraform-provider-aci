...
var _ datasource.DataSource = &FvEpIpTagDataSource{}
...
func (d *FvEpIpTagDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "Start metadata of datasource: aci_endpoint_tag_ip")
	resp.TypeName = req.ProviderTypeName + "_endpoint_tag_ip"
	tflog.Debug(ctx, "End metadata of datasource: aci_endpoint_tag_ip")
}

func (d *FvEpIpTagDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "Start schema of datasource: aci_endpoint_tag_ip")
	resp.Schema = schema.Schema{
		// This description is used by the documentation generator and the language server.
		MarkdownDescription: "The endpoint_tag_ip datasource for the 'fvEpIpTag' class",
	...
	tflog.Debug(ctx, "End schema of datasource: aci_endpoint_tag_ip")
}
...
func (d *FvEpIpTagDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "Start configure of datasource: aci_endpoint_tag_ip")
	...
	d.client = client
	tflog.Debug(ctx, "End configure of datasource: aci_endpoint_tag_ip")
}

func (d *FvEpIpTagDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "Start read of datasource: aci_endpoint_tag_ip")
	...
	tflog.Debug(ctx, fmt.Sprintf("Read of datasource aci_endpoint_tag_ip with id '%s'", data.Id.ValueString()))

	getAndSetFvEpIpTagAttributes(ctx, &resp.Diagnostics, d.client, data)

	if data.Id.IsNull() {
		resp.Diagnostics.AddError(
			"Failed to read aci_endpoint_tag_ip data source",
			fmt.Sprintf("The aci_endpoint_tag_ip data source with id '%s' has not been found", cachedId),
		)
		return
	}
	...
	tflog.Debug(ctx, fmt.Sprintf("End read of datasource aci_endpoint_tag_ip with id '%s'", data.Id.ValueString()))
}
