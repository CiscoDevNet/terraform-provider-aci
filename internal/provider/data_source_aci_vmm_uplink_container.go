...
var _ datasource.DataSource = &VmmUplinkPContDataSource{}
...
func (d *VmmUplinkPContDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "Start metadata of datasource: aci_vmm_uplink_container")
	resp.TypeName = req.ProviderTypeName + "_vmm_uplink_container"
	tflog.Debug(ctx, "End metadata of datasource: aci_vmm_uplink_container")
}

func (d *VmmUplinkPContDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "Start schema of datasource: aci_vmm_uplink_container")
	resp.Schema = schema.Schema{
		// This description is used by the documentation generator and the language server.
		MarkdownDescription: "The vmm_uplink_container datasource for the 'vmmUplinkPCont' class",
	...
	tflog.Debug(ctx, "End schema of datasource: aci_vmm_uplink_container")
}
...
func (d *VmmUplinkPContDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "Start configure of datasource: aci_vmm_uplink_container")
	...
	d.client = client
	tflog.Debug(ctx, "End configure of datasource: aci_vmm_uplink_container")
}

func (d *VmmUplinkPContDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "Start read of datasource: aci_vmm_uplink_container")
	...
	// Create a copy of the Id for when not found during getAndSetVmmUplinkPContAttributes
	cachedId := data.Id.ValueString()

	tflog.Debug(ctx, fmt.Sprintf("Read of datasource aci_vmm_uplink_container with id '%s'", data.Id.ValueString()))

	getAndSetVmmUplinkPContAttributes(ctx, &resp.Diagnostics, d.client, data)

	if data.Id.IsNull() {
		resp.Diagnostics.AddError(
			"Failed to read aci_vmm_uplink_container data source",
			fmt.Sprintf("The aci_vmm_uplink_container data source with id '%s' has not been found", cachedId),
		)
		return
	}
	...
	tflog.Debug(ctx, fmt.Sprintf("End read of datasource aci_vmm_uplink_container with id '%s'", data.Id.ValueString()))
}
