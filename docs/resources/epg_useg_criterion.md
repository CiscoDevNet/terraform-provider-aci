...
sidebar_current: "docs-aci-resource-aci_epg_useg_criterion"
description: |-
  Manages ACI EPG uSeg Criterion
---
...
The configuration snippet below creates a EPG uSeg Criterion with only required attributes.
...
```
The configuration snippet below shows all possible attributes of the EPG uSeg Criterion.
...
All examples for the EPG uSeg Criterion resource can be found in the [examples](https://github.com/CiscoDevNet/terraform-provider-aci/tree/master/examples/resources/aci_epg_useg_criterion) folder.
...
An existing EPG uSeg Criterion can be [imported](https://www.terraform.io/docs/import/index.html) into this resource with its distinguished name (DN), via the following command:

```
terraform import aci_epg_useg_criterion.example_application_epg uni/tn-{name}/ap-{name}/epg-{name}/crtrn
```

Starting in Terraform version 1.5, an existing EPG uSeg Criterion can be imported
using [import blocks](https://developer.hashicorp.com/terraform/language/import) via the following configuration:

```
import {
  id = "uni/tn-{name}/ap-{name}/epg-{name}/crtrn"
  to = aci_epg_useg_criterion.example_application_epg
}
```

## Child Resources
  
  - [aci_epg_useg_criterion_identity_group_attribute](https://registry.terraform.io/providers/CiscoDevNet/aci/latest/docs/resources/epg_useg_criterion_identity_group_attribute)
