...
sidebar_current: "docs-aci-list-resource-aci_tag"
description: |-
  Manages ACI Tag
---
...
List existing ACI Tag objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Tag with only required attributes. This snippet retrieves a list of all Tag objects.

```hcl
list "aci_tag" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Tag with filter capabilties.

```hcl
list "aci_tag" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(tagTag.dn,\"uni/tn-{name}/tagKey-{key}\")"
  }
}
```
