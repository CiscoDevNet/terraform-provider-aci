...
sidebar_current: "docs-aci-list-resource-aci_taboo_contract_subject"
description: |-
...
List existing ACI Taboo Contract Subject objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Taboo Contract Subject with only required attributes. This snippet retrieves a list of all Taboo Contract Subject objects.

```hcl
list "aci_taboo_contract_subject" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Taboo Contract Subject with filter capabilties.

```hcl
list "aci_taboo_contract_subject" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(vzTSubj.dn,\"uni/tn-{name}/taboo-{name}/tsubj-{name}\")"
  }
}
```
