...
sidebar_current: "docs-aci-list-resource-aci_relation_to_provided_contract"
description: |-
  Manages ACI Relation To Provided Contract
---

# aci_relation_to_provided_contract #

List existing ACI Relation To Provided Contract objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To Provided Contract with only required attributes. This snippet retrieves a list of all Relation To Provided Contract objects.

```hcl
list "aci_relation_to_provided_contract" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation To Provided Contract with filter capabilties.

```hcl
list "aci_relation_to_provided_contract" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsProv.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/rsprov-{tnVzBrCPName}\")"
  }
}
```
