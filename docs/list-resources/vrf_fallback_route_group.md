...
sidebar_current: "docs-aci-list-resource-aci_vrf_fallback_route_group"
description: |-
...
List existing ACI VRF Fallback Route Group objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for VRF Fallback Route Group with only required attributes. This snippet retrieves a list of all VRF Fallback Route Group objects.

```hcl
list "aci_vrf_fallback_route_group" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for VRF Fallback Route Group with filter capabilties.

```hcl
list "aci_vrf_fallback_route_group" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvFBRGroup.dn,\"uni/tn-{name}/ctx-{name}/fbrg-{name}\")"
  }
}
```
