...
sidebar_current: "docs-aci-list-resource-aci_endpoint_tag_mac"
description: |-
...
List existing ACI Endpoint Tag MAC objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Endpoint Tag MAC with only required attributes. This snippet retrieves a list of all Endpoint Tag MAC objects.

```hcl
list "aci_endpoint_tag_mac" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Endpoint Tag MAC with filter capabilties.

```hcl
list "aci_endpoint_tag_mac" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvEpMacTag.dn,\"uni/tn-{name}/eptags/epmactag-{mac}-[{bdName}]\")"
  }
}
```
