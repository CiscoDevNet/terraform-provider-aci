...
sidebar_current: "docs-aci-list-resource-aci_relation_from_attachable_access_entity_profile_to_domain"
description: |-
...
List existing ACI Relation From Attachable Access Entity Profile To Domain objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation From Attachable Access Entity Profile To Domain with only required attributes. This snippet retrieves a list of all Relation From Attachable Access Entity Profile To Domain objects.

```hcl
list "aci_relation_from_attachable_access_entity_profile_to_domain" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation From Attachable Access Entity Profile To Domain with filter capabilties.

```hcl
list "aci_relation_from_attachable_access_entity_profile_to_domain" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(infraRsDomP.dn,\"uni/infra/attentp-{name}/rsdomP-[{tDn}]\")"
  }
}
```
