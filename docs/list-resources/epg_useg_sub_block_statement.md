...
sidebar_current: "docs-aci-list-resource-aci_epg_useg_sub_block_statement"
description: |-
...
List existing ACI EPG uSeg Sub Block Statement objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for EPG uSeg Sub Block Statement with only required attributes. This snippet retrieves a list of all EPG uSeg Sub Block Statement objects.

```hcl
list "aci_epg_useg_sub_block_statement" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for EPG uSeg Sub Block Statement with filter capabilties.

```hcl
list "aci_epg_useg_sub_block_statement" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvSCrtrn.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/crtrn/crtrn-{name}\")"
  }
}
```
