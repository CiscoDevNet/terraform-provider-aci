...
sidebar_current: "docs-aci-list-resource-aci_netflow_monitor_policy"
description: |-
...
List existing ACI NetFlow Monitor Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for NetFlow Monitor Policy with only required attributes. This snippet retrieves a list of all NetFlow Monitor Policy objects.

```hcl
list "aci_netflow_monitor_policy" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for NetFlow Monitor Policy with filter capabilties.

```hcl
list "aci_netflow_monitor_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(netflowMonitorPol.dn,\"uni/infra/monitorpol-{name}\")"
  }
}
```
