...
sidebar_current: "docs-aci-list-resource-aci_relation_to_consumed_contract"
description: |-
...
List existing ACI Relation To Consumed Contract objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To Consumed Contract with only required attributes. This snippet retrieves a list of all Relation To Consumed Contract objects.

```hcl
list "aci_relation_to_consumed_contract" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation To Consumed Contract with filter capabilties.

```hcl
list "aci_relation_to_consumed_contract" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsCons.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/rscons-{tnVzBrCPName}\")"
  }
}
```
