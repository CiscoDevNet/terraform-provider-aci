...
sidebar_current: "docs-aci-list-resource-aci_l3out_redistribute_policy"
description: |-
...
List existing ACI L3Out Redistribute Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for L3Out Redistribute Policy with only required attributes. This snippet retrieves a list of all L3Out Redistribute Policy objects.

```hcl
list "aci_l3out_redistribute_policy" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for L3Out Redistribute Policy with filter capabilties.

```hcl
list "aci_l3out_redistribute_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(l3extRsRedistributePol.dn,\"uni/tn-{name}/out-{name}/rsredistributePol-[{tnRtctrlProfileName}]-{src}\")"
  }
}
```
