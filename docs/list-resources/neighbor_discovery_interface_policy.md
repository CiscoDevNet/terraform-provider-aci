...
sidebar_current: "docs-aci-list-resource-aci_neighbor_discovery_interface_policy"
description: |-
...
List existing ACI Neighbor Discovery Interface Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Neighbor Discovery Interface Policy with only required attributes. This snippet retrieves a list of all Neighbor Discovery Interface Policy objects.

```hcl
list "aci_neighbor_discovery_interface_policy" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Neighbor Discovery Interface Policy with filter capabilties.

```hcl
list "aci_neighbor_discovery_interface_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(ndIfPol.dn,\"uni/tn-{name}/ndifpol-{name}\")"
  }
}
```
