...
sidebar_current: "docs-aci-list-resource-aci_relation_to_contract_master"
description: |-
...
List existing ACI Relation To Contract Master objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To Contract Master with filter capabilties.

```hcl
list "aci_relation_to_contract_master" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsSecInherited.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/rssecInherited-[{tDn}]\")"
  }
}
```
