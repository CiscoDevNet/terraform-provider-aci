...
sidebar_current: "docs-aci-list-resource-aci_external_management_network_subnet"
description: |-
...
List existing ACI External Management Network Subnet objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for External Management Network Subnet with only required attributes. This snippet retrieves a list of all External Management Network Subnet objects.

```hcl
list "aci_external_management_network_subnet" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for External Management Network Subnet with filter capabilties.

```hcl
list "aci_external_management_network_subnet" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(mgmtSubnet.dn,\"uni/tn-mgmt/extmgmt-default/instp-{name}/subnet-[{ip}]\")"
  }
}
```
