...
sidebar_current: "docs-aci-list-resource-aci_relation_from_any_to_contract_interface"
description: |-
...
List existing ACI Relation From Any To Contract Interface objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation From Any To Contract Interface with only required attributes. This snippet retrieves a list of all Relation From Any To Contract Interface objects.

```hcl
list "aci_relation_from_any_to_contract_interface" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation From Any To Contract Interface with filter capabilties.

```hcl
list "aci_relation_from_any_to_contract_interface" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(vzRsAnyToConsIf.dn,\"uni/tn-{name}/ctx-{name}/any/rsanyToConsIf-{tnVzCPIfName}\")"
  }
}
```
