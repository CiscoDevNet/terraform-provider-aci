...
sidebar_current: "docs-aci-list-resource-aci_relation_from_vrf_to_address_family_ospf_timers"
description: |-
...
List existing ACI Relation From VRF To Address Family OSPF Timers objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation From VRF To Address Family OSPF Timers with only required attributes. This snippet retrieves a list of all Relation From VRF To Address Family OSPF Timers objects.

```hcl
list "aci_relation_from_vrf_to_address_family_ospf_timers" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation From VRF To Address Family OSPF Timers with filter capabilties.

```hcl
list "aci_relation_from_vrf_to_address_family_ospf_timers" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsCtxToOspfCtxPol.dn,\"uni/tn-{name}/ctx-{name}/rsctxToOspfCtxPol-[{tnOspfCtxPolName}]-{af}\")"
  }
}
```
