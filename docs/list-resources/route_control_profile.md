...
sidebar_current: "docs-aci-list-resource-aci_route_control_profile"
description: |-
...
List existing ACI Route Control Profile objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Route Control Profile with only required attributes. This snippet retrieves a list of all Route Control Profile objects.

```hcl
list "aci_route_control_profile" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Route Control Profile with filter capabilties.

```hcl
list "aci_route_control_profile" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(rtctrlProfile.dn,\"uni/tn-{name}/prof-{name}\")"
  }
}
```
