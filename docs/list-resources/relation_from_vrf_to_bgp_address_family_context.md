...
sidebar_current: "docs-aci-list-resource-aci_relation_from_vrf_to_bgp_address_family_context"
description: |-
...
List existing ACI Relation From VRF To BGP Address Family Context objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation From VRF To BGP Address Family Context with only required attributes. This snippet retrieves a list of all Relation From VRF To BGP Address Family Context objects.

```hcl
list "aci_relation_from_vrf_to_bgp_address_family_context" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation From VRF To BGP Address Family Context with filter capabilties.

```hcl
list "aci_relation_from_vrf_to_bgp_address_family_context" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsCtxToBgpCtxAfPol.dn,\"uni/tn-{name}/ctx-{name}/rsctxToBgpCtxAfPol-[{tnBgpCtxAfPolName}]-{af}\")"
  }
}
```
