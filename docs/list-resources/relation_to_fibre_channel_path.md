...
sidebar_current: "docs-aci-list-resource-aci_relation_to_fibre_channel_path"
description: |-
...
List existing ACI Relation To Fibre Channel Path objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To Fibre Channel Path with only required attributes. This snippet retrieves a list of all Relation To Fibre Channel Path objects.

```hcl
list "aci_relation_to_fibre_channel_path" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation To Fibre Channel Path with filter capabilties.

```hcl
list "aci_relation_to_fibre_channel_path" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsFcPathAtt.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/rsfcPathAtt-[{tDn}]\")"
  }
}
```
