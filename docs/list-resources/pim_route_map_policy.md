...
sidebar_current: "docs-aci-list-resource-aci_pim_route_map_policy"
description: |-
...
List existing ACI PIM Route Map Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for PIM Route Map Policy with only required attributes. This snippet retrieves a list of all PIM Route Map Policy objects.

```hcl
list "aci_pim_route_map_policy" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for PIM Route Map Policy with filter capabilties.

```hcl
list "aci_pim_route_map_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(pimRouteMapPol.dn,\"uni/tn-{name}/rtmap-{name}\")"
  }
}
```
