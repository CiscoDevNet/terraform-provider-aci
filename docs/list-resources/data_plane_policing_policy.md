...
sidebar_current: "docs-aci-list-resource-aci_data_plane_policing_policy"
description: |-
...
List existing ACI Data Plane Policing Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Data Plane Policing Policy with filter capabilties.

```hcl
list "aci_data_plane_policing_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(qosDppPol.dn,\"uni/infra/qosdpppol-{name}\")"
  }
}
```
