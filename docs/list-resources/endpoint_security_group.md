...
sidebar_current: "docs-aci-list-resource-aci_endpoint_security_group"
description: |-
...
List existing ACI Endpoint Security Group objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Endpoint Security Group with only required attributes. This snippet retrieves a list of all Endpoint Security Group objects.

```hcl
list "aci_endpoint_security_group" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Endpoint Security Group with filter capabilties.

```hcl
list "aci_endpoint_security_group" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvESg.dn,\"uni/tn-{name}/ap-{name}/esg-{name}\")"
  }
}
```
