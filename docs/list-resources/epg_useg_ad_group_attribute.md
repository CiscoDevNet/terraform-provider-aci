...
sidebar_current: "docs-aci-list-resource-aci_epg_useg_ad_group_attribute"
description: |-
...
List existing ACI EPG uSeg AD Group Attribute objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for EPG uSeg AD Group Attribute with only required attributes. This snippet retrieves a list of all EPG uSeg AD Group Attribute objects.

```hcl
list "aci_epg_useg_ad_group_attribute" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for EPG uSeg AD Group Attribute with filter capabilties.

```hcl
list "aci_epg_useg_ad_group_attribute" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvIdGroupAttr.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/crtrn/idgattr-[{selector}]\")"
  }
}
```
