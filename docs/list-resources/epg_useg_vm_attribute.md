...
sidebar_current: "docs-aci-list-resource-aci_epg_useg_vm_attribute"
description: |-
...
List existing ACI EPG uSeg VM Attribute objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for EPG uSeg VM Attribute with only required attributes. This snippet retrieves a list of all EPG uSeg VM Attribute objects.

```hcl
list "aci_epg_useg_vm_attribute" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for EPG uSeg VM Attribute with filter capabilties.

```hcl
list "aci_epg_useg_vm_attribute" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvVmAttr.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/crtrn/crtrn-{name}/vmattr-{name}\")"
  }
}
```
