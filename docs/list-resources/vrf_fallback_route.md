...
sidebar_current: "docs-aci-list-resource-aci_vrf_fallback_route"
description: |-
...
List existing ACI VRF Fallback Route objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for VRF Fallback Route with only required attributes. This snippet retrieves a list of all VRF Fallback Route objects.

```hcl
list "aci_vrf_fallback_route" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for VRF Fallback Route with filter capabilties.

```hcl
list "aci_vrf_fallback_route" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvFBRoute.dn,\"uni/tn-{name}/ctx-{name}/fbrg-{name}/pfx-[{fbrPrefix}]\")"
  }
}
```
