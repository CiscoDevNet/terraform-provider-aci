...
sidebar_current: "docs-aci-list-resource-aci_fabric_external_connection_policy"
description: |-
...
List existing ACI Fabric External Connection Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Fabric External Connection Policy with only required attributes. This snippet retrieves a list of all Fabric External Connection Policy objects.

```hcl
list "aci_fabric_external_connection_policy" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Fabric External Connection Policy with filter capabilties.

```hcl
list "aci_fabric_external_connection_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvFabricExtConnP.dn,\"uni/tn-{name}/fabricExtConnP-{id}\")"
  }
}
```
