...
sidebar_current: "docs-aci-list-resource-aci_leaf_interface_profile"
description: |-
...
List existing ACI Leaf Interface Profile objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Leaf Interface Profile with filter capabilties.

```hcl
list "aci_leaf_interface_profile" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(infraAccPortP.dn,\"uni/infra/accportprof-{name}\")"
  }
}
```
