...
sidebar_current: "docs-aci-list-resource-aci_vrf"
description: |-
  Manages ACI VRF
---
...
List existing ACI VRF objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for VRF with filter capabilties.

```hcl
list "aci_vrf" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvCtx.dn,\"uni/tn-{name}/ctx-{name}\")"
  }
}
```
