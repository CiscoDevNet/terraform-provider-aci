...
sidebar_current: "docs-aci-list-resource-aci_relation_to_consumed_out_of_band_contract"
description: |-
...
List existing ACI Relation To Consumed Out Of Band Contract objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To Consumed Out Of Band Contract with filter capabilties.

```hcl
list "aci_relation_to_consumed_out_of_band_contract" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(mgmtRsOoBCons.dn,\"uni/tn-{name}/extmgmt-{name}/instp-{name}/rsooBCons-{tnVzOOBBrCPName}\")"
  }
}
```
