...
sidebar_current: "docs-aci-list-resource-aci_l3out_node_sid_profile"
description: |-
...
List existing ACI L3Out Node SR-MPLS Segment ID Profile objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for L3Out Node SR-MPLS Segment ID Profile with only required attributes. This snippet retrieves a list of all L3Out Node SR-MPLS Segment ID Profile objects.

```hcl
list "aci_l3out_node_sid_profile" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for L3Out Node SR-MPLS Segment ID Profile with filter capabilties.

```hcl
list "aci_l3out_node_sid_profile" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(mplsNodeSidP.dn,\"uni/tn-{name}/out-{name}/lnodep-{name}/rsnodeL3OutAtt-[{tDn}]/lbp-[{addr}]/nodesidp-{sidoffset}\")"
  }
}
```
