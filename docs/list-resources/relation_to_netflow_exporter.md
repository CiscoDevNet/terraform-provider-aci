...
sidebar_current: "docs-aci-list-resource-aci_relation_to_netflow_exporter"
description: |-
...
List existing ACI Relation To NetFlow Exporter objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To NetFlow Exporter with only required attributes. This snippet retrieves a list of all Relation To NetFlow Exporter objects.

```hcl
list "aci_relation_to_netflow_exporter" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation To NetFlow Exporter with filter capabilties.

```hcl
list "aci_relation_to_netflow_exporter" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(netflowRsMonitorToExporter.dn,\"uni/infra/monitorpol-{name}/rsmonitorToExporter-{tnNetflowExporterPolName}\")"
  }
}
```
