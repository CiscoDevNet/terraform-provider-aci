...
sidebar_current: "docs-aci-list-resource-aci_relation_to_vrf_fallback_route_group"
description: |-
...
List existing ACI Relation To VRF Fallback Route Group objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To VRF Fallback Route Group with only required attributes. This snippet retrieves a list of all Relation To VRF Fallback Route Group objects.

```hcl
list "aci_relation_to_vrf_fallback_route_group" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation To VRF Fallback Route Group with filter capabilties.

```hcl
list "aci_relation_to_vrf_fallback_route_group" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(l3extRsOutToFBRGroup.dn,\"uni/tn-{name}/out-{name}/rsoutToFBRGroup-[{tDn}]\")"
  }
}
```
