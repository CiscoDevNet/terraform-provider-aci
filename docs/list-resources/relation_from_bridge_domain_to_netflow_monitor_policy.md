...
sidebar_current: "docs-aci-list-resource-aci_relation_from_bridge_domain_to_netflow_monitor_policy"
description: |-
...
List existing ACI Relation From Bridge Domain To NetFlow Monitor Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation From Bridge Domain To NetFlow Monitor Policy with filter capabilties.

```hcl
list "aci_relation_from_bridge_domain_to_netflow_monitor_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsBDToNetflowMonitorPol.dn,\"uni/tn-{name}/svcBD-{name}/rsBDToNetflowMonitorPol-[{tnNetflowMonitorPolName}]-{fltType}\")"
  }
}
```
