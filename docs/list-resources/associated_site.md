...
sidebar_current: "docs-aci-list-resource-aci_associated_site"
description: |-
...
List existing ACI Associated Site objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Associated Site with only required attributes. This snippet retrieves a list of all Associated Site objects.

```hcl
list "aci_associated_site" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Associated Site with filter capabilties.

```hcl
list "aci_associated_site" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvSiteAssociated.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/stAsc\")"
  }
}
```
