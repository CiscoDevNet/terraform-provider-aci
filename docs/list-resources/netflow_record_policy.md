...
sidebar_current: "docs-aci-list-resource-aci_netflow_record_policy"
description: |-
...
List existing ACI NetFlow Record Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for NetFlow Record Policy with only required attributes. This snippet retrieves a list of all NetFlow Record Policy objects.

```hcl
list "aci_netflow_record_policy" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for NetFlow Record Policy with filter capabilties.

```hcl
list "aci_netflow_record_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(netflowRecordPol.dn,\"uni/infra/recordpol-{name}\")"
  }
}
```
