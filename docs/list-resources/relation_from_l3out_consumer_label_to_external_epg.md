...
sidebar_current: "docs-aci-list-resource-aci_relation_from_l3out_consumer_label_to_external_epg"
description: |-
...
List existing ACI Relation From L3Out Consumer Label To External EPG objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation From L3Out Consumer Label To External EPG with filter capabilties.

```hcl
list "aci_relation_from_l3out_consumer_label_to_external_epg" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(l3extRsLblToInstP.dn,\"uni/tn-{name}/out-{name}/conslbl-{name}/rslblToInstP-[{tDn}]\")"
  }
}
```
