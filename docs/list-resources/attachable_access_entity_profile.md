...
sidebar_current: "docs-aci-list-resource-aci_attachable_access_entity_profile"
description: |-
...
List existing ACI Attachable Access Entity Profile objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Attachable Access Entity Profile with only required attributes. This snippet retrieves a list of all Attachable Access Entity Profile objects.

```hcl
list "aci_attachable_access_entity_profile" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Attachable Access Entity Profile with filter capabilties.

```hcl
list "aci_attachable_access_entity_profile" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(infraAttEntityP.dn,\"uni/infra/attentp-{name}\")"
  }
}
```
