...
sidebar_current: "docs-aci-list-resource-aci_vmm_credential"
description: |-
...
List existing ACI VMM Credential objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for VMM Credential with only required attributes. This snippet retrieves a list of all VMM Credential objects.

```hcl
list "aci_vmm_credential" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for VMM Credential with filter capabilties.

```hcl
list "aci_vmm_credential" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(vmmUsrAccP.dn,\"uni/vmmp-{vendor}/dom-{name}/usracc-{name}\")"
  }
}
```
