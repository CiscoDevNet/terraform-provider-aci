...
sidebar_current: "docs-aci-list-resource-aci_dot1p_classifier"
description: |-
...
List existing ACI Dot1p Classifier objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Dot1p Classifier with filter capabilties.

```hcl
list "aci_dot1p_classifier" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(qosDot1PClass.dn,\"uni/tn-{name}/qoscustom-{name}/dot1P-{from}-{to}\")"
  }
}
```
