...
sidebar_current: "docs-aci-list-resource-aci_out_of_band_contract"
description: |-
...
List existing ACI Out Of Band Contract objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Out Of Band Contract with only required attributes. This snippet retrieves a list of all Out Of Band Contract objects.

```hcl
list "aci_out_of_band_contract" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Out Of Band Contract with filter capabilties.

```hcl
list "aci_out_of_band_contract" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(vzOOBBrCP.dn,\"uni/tn-mgmt/oobbrc-{name}\")"
  }
}
```
