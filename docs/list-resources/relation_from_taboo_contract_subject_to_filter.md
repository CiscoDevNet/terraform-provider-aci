...
sidebar_current: "docs-aci-list-resource-aci_relation_from_taboo_contract_subject_to_filter"
description: |-
...
List existing ACI Relation From Taboo Contract Subject To Filter objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation From Taboo Contract Subject To Filter with filter capabilties.

```hcl
list "aci_relation_from_taboo_contract_subject_to_filter" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(vzRsDenyRule.dn,\"uni/tn-{name}/taboo-{name}/tsubj-{name}/rsdenyRule-{tnVzFilterName}\")"
  }
}
```
