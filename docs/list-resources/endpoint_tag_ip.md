...
sidebar_current: "docs-aci-list-resource-aci_endpoint_tag_ip"
description: |-
...
List existing ACI Endpoint Tag IP objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Endpoint Tag IP with only required attributes. This snippet retrieves a list of all Endpoint Tag IP objects.

```hcl
list "aci_endpoint_tag_ip" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Endpoint Tag IP with filter capabilties.

```hcl
list "aci_endpoint_tag_ip" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvEpIpTag.dn,\"uni/tn-{name}/eptags/epiptag-[{ip}]-{ctxName}\")"
  }
}
```
