...
sidebar_current: "docs-aci-list-resource-aci_relation_to_taboo_contract"
description: |-
...
List existing ACI Relation To Taboo Contract objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To Taboo Contract with filter capabilties.

```hcl
list "aci_relation_to_taboo_contract" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsProtBy.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/rsprotBy-{tnVzTabooName}\")"
  }
}
```
