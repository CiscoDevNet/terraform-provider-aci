...
sidebar_current: "docs-aci-list-resource-aci_application_profile"
description: |-
...
List existing ACI Application Profile objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Application Profile with only required attributes. This snippet retrieves a list of all Application Profile objects.

```hcl
list "aci_application_profile" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Application Profile with filter capabilties.

```hcl
list "aci_application_profile" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvAp.dn,\"uni/tn-{name}/ap-{name}\")"
  }
}
```
