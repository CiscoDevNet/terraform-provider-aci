...
sidebar_current: "docs-aci-list-resource-aci_bridge_domain"
description: |-
...
List existing ACI Bridge Domain objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Bridge Domain with only required attributes. This snippet retrieves a list of all Bridge Domain objects.

```hcl
list "aci_bridge_domain" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Bridge Domain with filter capabilties.

```hcl
list "aci_bridge_domain" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvBD.dn,\"uni/tn-{name}/BD-{name}\")"
  }
}
```
