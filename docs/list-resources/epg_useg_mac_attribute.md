...
sidebar_current: "docs-aci-list-resource-aci_epg_useg_mac_attribute"
description: |-
...
List existing ACI EPG uSeg MAC Attribute objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for EPG uSeg MAC Attribute with only required attributes. This snippet retrieves a list of all EPG uSeg MAC Attribute objects.

```hcl
list "aci_epg_useg_mac_attribute" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for EPG uSeg MAC Attribute with filter capabilties.

```hcl
list "aci_epg_useg_mac_attribute" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvMacAttr.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/crtrn/macattr-{name}\")"
  }
}
```
