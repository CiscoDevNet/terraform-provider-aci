...
sidebar_current: "docs-aci-list-resource-aci_custom_qos_policy"
description: |-
...
List existing ACI Custom QoS Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Custom QoS Policy with filter capabilties.

```hcl
list "aci_custom_qos_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(qosCustomPol.dn,\"uni/tn-{name}/qoscustom-{name}\")"
  }
}
```
