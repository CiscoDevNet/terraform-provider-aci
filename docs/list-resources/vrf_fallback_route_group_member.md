...
sidebar_current: "docs-aci-list-resource-aci_vrf_fallback_route_group_member"
description: |-
...
List existing ACI VRF Fallback Route Group Member objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for VRF Fallback Route Group Member with only required attributes. This snippet retrieves a list of all VRF Fallback Route Group Member objects.

```hcl
list "aci_vrf_fallback_route_group_member" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for VRF Fallback Route Group Member with filter capabilties.

```hcl
list "aci_vrf_fallback_route_group_member" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvFBRMember.dn,\"uni/tn-{name}/ctx-{name}/fbrg-{name}/nexthop-[{rnhAddr}]\")"
  }
}
```
