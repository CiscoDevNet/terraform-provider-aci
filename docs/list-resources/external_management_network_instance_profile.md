...
sidebar_current: "docs-aci-list-resource-aci_external_management_network_instance_profile"
description: |-
...
List existing ACI External Management Network Instance Profile objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for External Management Network Instance Profile with only required attributes. This snippet retrieves a list of all External Management Network Instance Profile objects.

```hcl
list "aci_external_management_network_instance_profile" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for External Management Network Instance Profile with filter capabilties.

```hcl
list "aci_external_management_network_instance_profile" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(mgmtInstP.dn,\"uni/tn-mgmt/extmgmt-default/instp-{name}\")"
  }
}
```
