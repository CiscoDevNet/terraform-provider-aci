...
sidebar_current: "docs-aci-list-resource-aci_igmp_snooping_policy"
description: |-
...
List existing ACI IGMP Snooping Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for IGMP Snooping Policy with filter capabilties.

```hcl
list "aci_igmp_snooping_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(igmpSnoopPol.dn,\"uni/tn-{name}/snPol-{name}\")"
  }
}
```
