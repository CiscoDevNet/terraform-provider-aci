...
sidebar_current: "docs-aci-list-resource-aci_first_hop_security_policy"
description: |-
...
List existing ACI First Hop Security Policy objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for First Hop Security Policy with only required attributes. This snippet retrieves a list of all First Hop Security Policy objects.

```hcl
list "aci_first_hop_security_policy" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for First Hop Security Policy with filter capabilties.

```hcl
list "aci_first_hop_security_policy" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fhsBDPol.dn,\"uni/tn-{name}/bdpol-{name}\")"
  }
}
```
