---
# Documentation generated by "gen/generator.go"; DO NOT EDIT.
# In order to regenerate this file execute `go generate` from the repository root.
# More details can be found in the [README](https://github.com/CiscoDevNet/terraform-provider-aci/blob/master/README.md).
subcategory: "Tenant Policies"
layout: "aci"
page_title: "ACI: aci_ip_sla_track_list"
sidebar_current: "docs-aci-list-resource-aci_ip_sla_track_list"
description: |-
  Manages ACI IP SLA Track List
---

# aci_ip_sla_track_list #

List existing ACI IP SLA Track List objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.

For more information about the usage see the official Hashicorp documentation for [importing existing resources in bulk](https://developer.hashicorp.com/terraform/language/v1.14.x/import/bulk?page=import&page=bulk).

## API Information ##

* Class: [fvTrackList](https://pubhub.devnetcloud.com/media/model-doc-latest/docs/app/index.html#/objects/fvTrackList/overview)

* Supported in ACI versions: 4.1(1i) and later.

* Distinguished Name Format: `uni/tn-{name}/tracklist-{name}`

## GUI Information ##

* Location: `Tenants -> Policies -> Protocol -> IP SLA -> Track Lists`

## Example Usage ##

The configuration snippet below shows an example for IP SLA Track List with only required attributes. This snippet retrieves a list of all IP SLA Track List objects.

```hcl
list "aci_ip_sla_track_list" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for IP SLA Track List with filter capabilties.

```hcl
list "aci_ip_sla_track_list" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvTrackList.dn,\"uni/tn-{name}/tracklist-{name}\")"
  }
}
```

## Schema ##

### Optional ###

* `config` - (map) A map that contains all configurable options for the list resource.
    
    ### Optional ###

    * `filter` - (string) A filter to restrict the results of the class query to APIC. See [Cisco APIC REST API Configuration Guide, Release 4.2(x) and Later](https://www.cisco.com/c/en/us/td/docs/dcn/aci/apic/all/apic-rest-api-configuration-guide/cisco-apic-rest-api-configuration-guide-42x-and-later/m_using_the_rest_api.html#concept_1B75A78853DD46AABC15B048AAFAD1AD) for more information about composing filter query expressions.
