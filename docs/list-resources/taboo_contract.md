...
sidebar_current: "docs-aci-list-resource-aci_taboo_contract"
description: |-
...
List existing ACI Taboo Contract objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Taboo Contract with only required attributes. This snippet retrieves a list of all Taboo Contract objects.

```hcl
list "aci_taboo_contract" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Taboo Contract with filter capabilties.

```hcl
list "aci_taboo_contract" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(vzTaboo.dn,\"uni/tn-{name}/taboo-{name}\")"
  }
}
```
