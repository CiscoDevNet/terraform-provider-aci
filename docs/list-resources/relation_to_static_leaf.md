...
sidebar_current: "docs-aci-list-resource-aci_relation_to_static_leaf"
description: |-
...
List existing ACI Relation To Static Leaf objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To Static Leaf with only required attributes. This snippet retrieves a list of all Relation To Static Leaf objects.

```hcl
list "aci_relation_to_static_leaf" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation To Static Leaf with filter capabilties.

```hcl
list "aci_relation_to_static_leaf" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsNodeAtt.dn,\"uni/tn-{name}/ap-{name}/epg-{name}/rsnodeAtt-[{tDn}]\")"
  }
}
```
