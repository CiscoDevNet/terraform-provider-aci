...
sidebar_current: "docs-aci-list-resource-aci_fex_profile"
description: |-
...
List existing ACI FEX Profile objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for FEX Profile with only required attributes. This snippet retrieves a list of all FEX Profile objects.

```hcl
list "aci_fex_profile" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for FEX Profile with filter capabilties.

```hcl
list "aci_fex_profile" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(infraFexP.dn,\"uni/infra/fexprof-{name}\")"
  }
}
```
