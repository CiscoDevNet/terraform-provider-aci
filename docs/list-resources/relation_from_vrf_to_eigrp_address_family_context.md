...
sidebar_current: "docs-aci-list-resource-aci_relation_from_vrf_to_eigrp_address_family_context"
description: |-
...
List existing ACI Relation From VRF To EIGRP Address Family Context objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation From VRF To EIGRP Address Family Context with filter capabilties.

```hcl
list "aci_relation_from_vrf_to_eigrp_address_family_context" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsCtxToEigrpCtxAfPol.dn,\"uni/tn-{name}/ctx-{name}/rsctxToEigrpCtxAfPol-[{tnEigrpCtxAfPolName}]-{af}\")"
  }
}
```
