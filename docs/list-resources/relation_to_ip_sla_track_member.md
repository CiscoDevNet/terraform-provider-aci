...
sidebar_current: "docs-aci-list-resource-aci_relation_to_ip_sla_track_member"
description: |-
...
List existing ACI Relation To IP SLA Track Member objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation To IP SLA Track Member with only required attributes. This snippet retrieves a list of all Relation To IP SLA Track Member objects.

```hcl
list "aci_relation_to_ip_sla_track_member" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Relation To IP SLA Track Member with filter capabilties.

```hcl
list "aci_relation_to_ip_sla_track_member" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(fvRsOtmListMember.dn,\"uni/tn-{name}/tracklist-{name}/rsotmListMember-[{tDn}]\")"
  }
}
```
