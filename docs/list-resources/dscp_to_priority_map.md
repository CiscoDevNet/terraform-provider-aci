...
sidebar_current: "docs-aci-list-resource-aci_dscp_to_priority_map"
description: |-
...
List existing ACI DSCP to Priority Map objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for DSCP to Priority Map with only required attributes. This snippet retrieves a list of all DSCP to Priority Map objects.

```hcl
list "aci_dscp_to_priority_map" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for DSCP to Priority Map with filter capabilties.

```hcl
list "aci_dscp_to_priority_map" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(qosDscpClass.dn,\"uni/tn-{name}/qoscustom-{name}/dcsp-{from}-{to}\")"
  }
}
```
