...
sidebar_current: "docs-aci-list-resource-aci_imported_logical_device"
description: |-
...
List existing ACI Imported Logical Device objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Imported Logical Device with only required attributes. This snippet retrieves a list of all Imported Logical Device objects.

```hcl
list "aci_imported_logical_device" "example" {
  provider = aci
}
```

The configuration snippet below shows an example for Imported Logical Device with filter capabilties.

```hcl
list "aci_imported_logical_device" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(vnsLDevIf.dn,\"uni/tn-{name}/lDevIf-[{ldev}]\")"
  }
}
```
