...
sidebar_current: "docs-aci-list-resource-aci_relation_from_any_to_provider_contract"
description: |-
...
List existing ACI Relation From Any To Provider Contract objects with the [terraform query](https://developer.hashicorp.com/terraform/cli/commands/query) command.
...
The configuration snippet below shows an example for Relation From Any To Provider Contract with filter capabilties.

```hcl
list "aci_relation_from_any_to_provider_contract" "example" {
  provider = aci
  config {
    filter = "query-target-filter=eq(vzRsAnyToProv.dn,\"uni/tn-{name}/ctx-{name}/any/rsanyToProv-{tnVzBrCPName}\")"
  }
}
```
